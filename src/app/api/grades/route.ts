...
export async function GET() {
  ...
  try {
    ...
    console.log(`[Grades GET] Fetched ${grades.length} reports in ${(performance.now() - startTime).toFixed(0)}ms`)
    ...
    const results = grades.map(grade => {
      ...
      positionCategory: (grade as any).positionCategory || null,
      positionAttributes: (grade as any).positionAttributes || null,
      // Admin edit tracking
    ...
    console.error(`[Grades GET] FAILED in ${(performance.now() - startTime).toFixed(0)}ms:`, error)
